...
import {
  ArrowLeft, Building2, RefreshCw, Trash2, Upload, X, Plus, Tag, CheckCircle2, Eye,
} from 'lucide-react';
...
const ParishDetail: React.FC = () => {
  ...
  async function saveInfo(e: React.SyntheticEvent) {
    e.preventDefault();
    if (!infoForm.name.trim()) { setInfoError('Nome é obrigatório'); return; }
    setSavingInfo(true); setInfoError(''); setInfoSuccess('');
    try {
      await ParishService.put(id, { name: infoForm.name.trim(), active: infoForm.active });
  ...
  async function handleLogoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      await ParishService.uploadLogo(id, file);
      queryClient.invalidateQueries({ queryKey: queryKeys.parishes.detail(id) });
  ...
  async function handleDelete() {
    ...
    try {
      ...
      router.push(parish?.sector_id ? `/app/sectors/${parish?.sector_id}` : '/app/parishes');
    } catch (err: unknown) {
      setDeleteError((err as { data?: { message?: string } })?.data?.message ?? 'Erro ao excluir paróquia.');
  ...
  const backHref = parish.sector_id ? `/app/sectors/${parish.sector_id}` : '/app/parishes';
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted mt-0.5 flex items-center gap-1">
              ...
              {parish.sector && (<><span>·</span><span>{parish.sector.name}</span></>)}
            </p>
          </div>
        </div>
      ...
      <SectionCard title="Informações Básicas">
        <form onSubmit={saveInfo} noValidate className="space-y-5">
          <Input
            label="Nome *" name="name" placeholder="Nome da paróquia"
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-text-muted mt-0.5">Paróquias inativas não aparecem para seleção</p>
            </div>
            <Toggle checked={infoForm.active} onChange={() => setInfoForm((p) => ({ ...p, active: !p.active }))} />
          </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-panel border border-border rounded-xl p-6 w-full max-w-sm mx-4 space-y-4">
            <div>
              <h3 className="text-base font-bold text-text">Excluir paróquia?</h3>
              <p className="text-sm text-text-muted mt-1">
                Esta ação é irreversível. Todos os usuários, pessoas e encontros vinculados serão afetados.
              </p>
            </div>
