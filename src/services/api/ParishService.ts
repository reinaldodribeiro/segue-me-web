...
class ParishService extends CrudService<Parish, ParishPayload> {
  ...
  listBySector(sectorId: string, params?: Record<string, unknown>): Promise<AxiosResponse<PaginatedResponse<Parish>>> {
    return api.get(`sectors/${sectorId}/parishes`, { params });
  }

  /** POST /sectors/{sector}/parishes */
  createInSector(sectorId: string, data: ParishPayload): Promise<AxiosResponse<{ data: Parish }>> {
    return api.post(`sectors/${sectorId}/parishes`, data);
  }

  /** PUT /parishes/{parish} — backend expects PUT, not PATCH */
  put(id: string, data: Partial<ParishPayload & { active: boolean }>): Promise<AxiosResponse<{ data: Parish }>> {
  ...
  addSkill(id: string, skill: string): Promise<AxiosResponse<{ data: string[] }>> {
    return api.post(`parishes/${id}/skills`, { skill });
  }

  /** DELETE /parishes/{parish}/skills */
  removeSkill(id: string, skill: string): Promise<AxiosResponse<{ data: string[] }>> {
    return api.delete<{ data: string[] }>(`parishes/${id}/skills`, { data: { skill } });
  }
}
