...
/**
 * Next.js Proxy — authentication gate.
 *
 ...
 *     The Proxy runs on the Node.js runtime and does not have access to the
 *     full user object (roles, parish_id, etc.). Those checks live in
 *     PermissionGuard (client) and in the Laravel backend (policies).
 *
 * Token storage note:
 *   The current AuthContext stores the token in localStorage under the key
 *   "authToken". The Proxy cannot read localStorage — it only has access to
